...
public partial class Expenses : System.Web.UI.Page
{
    ...
    void RunUSers()
    {
        ...
        string Getqry = "";
        Getqry = "SELECT  intid, incometype, ServiceName,NameOfIncome,amount,CONVERT(Varchar(16),offeringDate,106),capturedBy FROM Offering WHERE IsExpense='1' and status = '1' and   ChurchID = '" + Session["ChurchID"].ToString() + "' and campus = '" + Session["Campus"].ToString() + "' ORDER BY  intid DESC";
        ...
        if (table.Rows.Count > 0)
        {
            foreach (DataRow Row in table.Rows)
            {
                ...
                //float F = float.Parse(GetAmount, CultureInfo.InvariantCulture.NumberFormat);
                //decimal dvalue = Convert.ToDecimal(F);
                // dvalue =  Decimal.Round(dvalue, 2);


                // GetAmount = dvalue.ToString();
                htmltext += " <tr> " +
    ...
    protected void btnViewMember_ServerClick(object sender, EventArgs e)
    {
        ...
        if (tb.Rows.Count > 0)
        {
            foreach (DataRow rows in tb.Rows)
            {
                ...
                if (EditCmdType.SelectedValue.ToString() == "Other")
                {
    ...
    protected void CmdType_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (CmdType.SelectedValue.ToString() == "Other")
        {
