...
public partial class ManageGroup : System.Web.UI.Page
{
    ...
    void RunUSers()
    {
        ...
        string Getqry = "SELECT  intid,  groupname FROM ChurchGroup WHERE churchid = '" + Session["ChurchID"].ToString() + "' and campus = '" + Session["Campus"].ToString() + "'";
        ...
        if (table.Rows.Count > 0)
        {
            foreach (DataRow Row in table.Rows)
            {
                ...
                htmltext += " <tr> " +
                                 ...
                                 "<td ><center>  <a onclick='BtnEditMembers(this.id);' id='" + Row[0].ToString() + "' class='btn btn-secondary'> View </a>   &nbsp;	&nbsp;  <a onclick='BtnArchMem(this.id);' id='" + Row[0].ToString() + "' class='btn btn-secondary'> Remove </a>  </center></td>" +
    ...
    protected void btnViewMember_ServerClick(object sender, EventArgs e)
    {
        ...
        txtEditZone.Value = connect.SingleRespSQL("SELECT  groupname  FROM  ChurchGroup  WHERE intid = '" + MemberID.Value + "'");
        BackDiv.Visible = true;
        BackDivs.Visible = false;
        EditZoneName.Visible = true;
    ...
    protected void btnArchive_ServerClick(object sender, EventArgs e)
    {
        ...
        int complete = connect.SingleIntSQL("DELETE FROM ChurchGroup WHERE intid = '" + MemberID.Value + "' ");
        if (complete > 0)
        {
            RunUSers();
            RemoveNotie();
    ...
    protected void btnSaveCampus_ServerClick(object sender, EventArgs e)
    {
        ...
        int complete = connect.SingleIntSQL("INSERT INTO ChurchGroup (groupname,Churchid,Campus,createdby,createdDate)VALUES ('" + txtZoneName.Value + "','" + Session["ChurchID"].ToString() + "', '" + Session["Campus"].ToString() + "','" + Session["FullName"].ToString() + "',GETDATE())");
         if (complete > 0)
         {
    ...
    protected void btnUpdateName_ServerClick(object sender, EventArgs e)
    {
        if (txtEditZone.Value == "")
        {
            NotCompleteNotie();
            return;
        }
        string OldName = connect.SingleRespSQL("SELECT  groupname  FROM  ChurchGroup  WHERE intid = '" + MemberID.Value + "'");
        int complete = connect.SingleIntSQL("UPDATE  ChurchGroup SET groupname = '" + txtEditZone.Value + "' WHERE intid = '" + MemberID.Value + "' ");
        if (complete > 0)
        {

            connect.SingleIntSQL("UPDATE   Stats_Form SET ChurchGroup = '" + txtEditZone.Value + "'  WHERE ChurchID = '" + Session["ChurchID"].ToString() + "' and Campus = '" + Session["Campus"].ToString() + "' and ChurchGroup = '" + OldName + "'");

            RunUSers();
            BackDiv.Visible = false;
            BackDivs.Visible = true;
            EditZoneName.Visible = false;
