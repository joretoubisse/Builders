...
public partial class NewMembers : System.Web.UI.Page
{
    ...
    void RunMembers()
    {
        ...
        if (Session["ShowAll"].ToString() == "Yes")
        {
            Getqry = "SELECT intid, Name + ' ' + Surname,gender,Ministries,Celno,MemberNo FROM Stats_Form WHERE ChurchID = '" + Session["ChurchID"].ToString() + "'  and IsActive = '1' and Campus = '" + Session["Campus"].ToString() + "' and MemberType = 'NewMember'  ORDER BY  Name + ' ' + Surname ASC";
        }
        else
        {
            Getqry = "SELECT intid, Name + ' ' + Surname,gender,Ministries,Celno,MemberNo FROM Stats_Form WHERE ChurchID = '" + Session["ChurchID"].ToString() + "'  and IsActive = '1' and MemberType = 'NewMember'  ORDER BY  Name + ' ' + Surname ASC";
        }
