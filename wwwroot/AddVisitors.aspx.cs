...
public partial class AddVisitors : System.Web.UI.Page
{
    ...
    protected void BtnSave_ServerClick(object sender, EventArgs e)
    {
       ...
       string sql = "INSERT Into Stats_Form (Idnumber, Surname, Name, Knownas, Gender, DOB, Ward, MaritalStat, Occupation,EmployerCheck, Skills, ResidentAdd, TelnoH, TelnoW,Fax,Celno, EmailAdd, Ministries,MemberNo,ChurchID,CapturedBy,DateCaptured,IsActive,ChurchInvolvement,ChurchStatus,DateMemberObtained,SpiritualGifts,FFS,Membertype) " +
           "VALUES ('" + textfield.Value + "','" + txtSurname.Value + "', '" + txtName.Value + "', '" + txtKnownAS.Value + "', '" + CmdGender.Value + "', '" + datepicker.Value + "', '" + CmdWard.Value + "', '" + CmdMarital.Value + "', '" + txtOccupation.Value + "', '" + txtEmployer.Value + "', '" + txtSkills.Value + "', '" + txtAddress.Value + "', '" + txtTellH.Value + "', '" + txtTellW.Value + "', '" + txtFax.Value + "', '" + txtCellNo.Value + "', '" + txtEmail.Value + "', '" + CmdMinistries.Value + "', '" + MemberNo + "', '" + Session["ChurchID"].ToString() + "', '" + Session["FullName"].ToString() + "',GETDATE(),'1','" + txtChurchInv.Value + "','" + CmdStatus.Value + "','" + txtMemberObtained.Value + "','" + txtSpiritualGifts.Value + "','" + CmdFFS.Value + "','Visitor')";
    ...
    protected void BtnSaveMembers_ServerClick(object sender, EventArgs e)
    {
        ...
        #region Text Box not complete
        if ((txtName.Value == "") || (txtSurname.Value == "") || (txtAddress.Value == "") || (CmdGender.Value == "none") || (CmdSms.Value == "none") || (CmdBornAgain.Value == "nones") || (CmdCampus.Value == "none") ) 
        {
            NotCompleteNotie();
            return;
        }
        ...
        if (txtFirstCall.Value == "")
        {
            txtFirstCall.Value = null;
        }
        if (txtDateSalvation.Value == "")
        {
            txtDateSalvation.Value = null;
        }
      
        #region Save in SQL
       
        string sql = "INSERT Into Stats_Form (Idnumber, Surname, Name, Knownas, Gender, DOB, Ward, MaritalStat, Occupation,EmployerCheck, Skills, ResidentAdd, TelnoH, TelnoW,Fax,Celno, EmailAdd, Ministries,MemberNo,ChurchID,CapturedBy,DateCaptured,IsActive,marrieddate,churchzone,membertype,sms,newbeliever,iconnect,baptized,iserve,comments,GrowthPath,dateofVisit,DateofSalvation,Firstcall,visit,Campus,isBornAgain,TypeOfService,ServiceDate) " +
        "VALUES ('" + textfield.Value + "','" + txtSurname.Value + "', '" + txtName.Value + "', '" + txtKnownAS.Value + "', '" + CmdGender.Value + "', '" + datepicker.Value + "', '" + CmdWard.Value + "', '" + CmdMarital.Value + "', '" + txtOccupation.Value + "', '" + txtEmployer.Value + "', '" + txtSkills.Value + "', '" + RemoveSpecialChars(txtAddress.Value) + "', '" + txtTellH.Value + "', '" + txtTellW.Value + "', '" + txtFax.Value + "', '" + txtCellNo.Value + "', '" + txtEmail.Value + "', '" + CmdMinistries.Value + "', '" + MemberNo + "', '" + Session["ChurchID"].ToString() + "', '" + Session["FullName"].ToString() + "',GETDATE(),'1','" + txtMarriageDate.Value + "','" + CmdZone.Value + "','Visitor','" + CmdSms.Value + "','" + txtNewBeliever.Value + "','" + txtIconnect.Value + "','" + txtBaptism.Value + "','" + RemoveSpecialChars(txtiServe.Value) + "','" + RemoveSpecialChars(txtComment.Value) + "','" + CmdGrowthPath.Value + "','" + txtFirstVisit.Value + "','" + txtDateSalvation.Value + "','" + txtFirstCall.Value + "','" + RemoveSpecialChars(txtVisit.Value) + "','" + CmdCampus.Value + "','" + CmdBornAgain.Value + "','" + CmdService.SelectedValue.ToString() + "','" + txtExperienceDate.Value + "')";
