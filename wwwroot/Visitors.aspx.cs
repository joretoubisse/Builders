...
public partial class Visitors : System.Web.UI.Page
{
    ...
    void RunMembers()
    {
        ...
        if (Session["ShowAll"].ToString() == "Yes")
        {
            Getqry = "SELECT intid, Name + ' ' + Surname,gender,Ministries,Celno,MemberNo,CONVERT(varchar(16),Firstcall,106),CONVERT(varchar(16),FirstEmail,106) FROM Stats_Form WHERE ChurchID = '" + Session["ChurchID"].ToString() + "'  and IsActive = '1' and MemberType = 'Visitor'  ORDER BY  Name + ' ' + Surname ASC";
        }
        else
        {
            Getqry = "SELECT intid, Name + ' ' + Surname,gender,Ministries,Celno,MemberNo,CONVERT(varchar(16),Firstcall,106),CONVERT(varchar(16),FirstEmail,106) FROM Stats_Form WHERE ChurchID = '" + Session["ChurchID"].ToString() + "'  and IsActive = '1' and MemberType = 'Visitor' and Campus = '" + Session["Campus"].ToString() + "'  ORDER BY  Name + ' ' + Surname ASC";
        }
        ...
        if (table.Rows.Count > 0)
        {
            foreach (DataRow Row in table.Rows)
            {
                ...
                string DatefirstCall = "";
                logthefile(Row[6].ToString());
                if (Row[6].ToString() == "01 Jan 1900")
                {
                    DatefirstCall = "";
                }
                else
                {
                    DatefirstCall = Row[6].ToString();
                }
    ...
    {
        int complete = connect.SingleIntSQL("UPDATE  Stats_Form SET MemberType = 'NewMember' WHERE ChurchID = '" + Session["ChurchID"].ToString() + "' and intid = '" + MemberID.Value + "' ");
        if (complete > 0)
        {
            RunMembers();
