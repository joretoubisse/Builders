...
public partial class ManageZones : System.Web.UI.Page
{
    ...
    void RunUSers()
    {
        ...
        string Getqry = "SELECT  intid,  Zone FROM ChurchZone WHERE churchid = '" + Session["ChurchID"].ToString() + "' and campus = '" + Session["Campus"].ToString() + "'";
    ...
    protected void btnViewMember_ServerClick(object sender, EventArgs e)
    {
        ...
        txtEditZone.Value = connect.SingleRespSQL("SELECT  Zone  FROM  ChurchZone  WHERE intid = '" + MemberID.Value + "'");
    ...
    protected void btnArchive_ServerClick(object sender, EventArgs e)
    {
        ...
        int complete = connect.SingleIntSQL("DELETE FROM ChurchZone WHERE intid = '" + MemberID.Value + "' ");
        if (complete > 0)
        {
            RunUSers();
            RemoveNotie();
    ...
    protected void btnSaveCampus_ServerClick(object sender, EventArgs e)
    {
        ...
        int complete = connect.SingleIntSQL("INSERT INTO ChurchZone (Zone,Churchid,Campus,createdby,createdDate)VALUES ('" + txtZoneName.Value + "','" + Session["ChurchID"].ToString() + "', '" + Session["Campus"].ToString() + "','" + Session["FullName"].ToString() + "',GETDATE())");
         if (complete > 0)
         {
    ...
    protected void btnUpdateName_ServerClick(object sender, EventArgs e)
    {
        ...
        string OldName = connect.SingleRespSQL("SELECT  Zone  FROM  ChurchZone  WHERE intid = '" + MemberID.Value + "'");

        int complete = connect.SingleIntSQL("UPDATE  ChurchZone SET Zone = '" + txtEditZone.Value + "' WHERE intid = '" + MemberID.Value + "' ");
        if (complete > 0)
        {


            connect.SingleIntSQL("UPDATE   Stats_Form SET Churchzone = '" + txtEditZone.Value + "'  WHERE ChurchID = '" + Session["ChurchID"].ToString() + "' and Campus = '" + Session["Campus"].ToString() + "' and Churchzone = '" + OldName + "'");
